...
import { ASM_LAUNCH_PHASES, ASM_LAUNCH_SCAN_LINES } from '../../data/asmLaunchAnimationContent'
...
type Props = {
  /** When true, scan lines cycle forever. When false, stops on last line (fullscreen redirect). */
  loop?: boolean
  /** Fired once after the final line is shown (only when loop is false). */
  onComplete?: () => void
  className?: string
  /** Smaller type and spacing for embedding inside the portal preview chrome. */
  compact?: boolean
}

const scanLines = ASM_LAUNCH_SCAN_LINES
const phases = ASM_LAUNCH_PHASES

export function AsmPortalLaunchUI({ loop = true, onComplete, className = '', compact = false }: Props) {
    ...
    <div className={`text-center ${compact ? 'px-0 py-1' : ''} ${className}`}>
      ...
      <div className={`mx-auto w-full ${compact ? 'mt-1.5 max-w-none px-0' : 'mt-8 max-w-xl md:mt-10'}`}>
        ...
        >
          ...
          <span>{atEnd && !loop ? 'Ready.' : 'SCANNING.'}</span>
        </div>
      </div>
