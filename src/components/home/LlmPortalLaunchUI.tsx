...
import { LLM_LAUNCH_PHASES, LLM_LAUNCH_SCAN_LINES } from '../../data/llmLaunchAnimationContent'

export const LLM_ACCENT_TEXT: CSSProperties = {
...
const scanLines = LLM_LAUNCH_SCAN_LINES
const phases = LLM_LAUNCH_PHASES

export function LlmPortalLaunchUI({ loop = true, onComplete, className = '', compact = false }: Props) {
  ...
  const progressStatus = atEnd && !loop ? 'Ready.' : 'PROBING.'
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-heading">LLM Attack </span>
        <span className="text-brand">Suite</span>
      </h2>
      ...
      <nav
        ...
        aria-label="LLM Attack Suite launch phases"
      >
        {phases.map((p, i) => {
