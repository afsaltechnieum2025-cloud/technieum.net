...
import {
  AI_SEC_CHALLENGE,
  AI_SEC_CTA,
  AI_SEC_DELIVERABLE_REPORTS,
  AI_SEC_DELIVERABLES_PROGRAM_EXTRA,
  AI_SEC_EDGE,
  AI_SEC_HERO,
  AI_SEC_INDUSTRY_VERTICALS,
  AI_SEC_KEY_BENEFITS,
  AI_SEC_METHODOLOGY,
  AI_SEC_PARTNER,
  AI_SEC_SERVICE_CATALOG,
  AI_SEC_SOLUTION_INTRO,
} from '../data/aiSecurityPageContent'
import { aiSecurityHubConfig } from '../data/programHubRegistry'

export function AiSecurityPage() {
  const { visualNodes, visualAriaLabel, pillars, deliverables } = aiSecurityHubConfig
  const programPortalLines = [...deliverables, ...AI_SEC_DELIVERABLES_PROGRAM_EXTRA]
  ...
  return (
    <main id="main-content" className="flex flex-col bg-page">
        ...
        <div className="container relative z-10">
          <div className="grid items-start gap-6 lg:grid-cols-2 lg:gap-8">
            <div>
              ...
              <p className="mb-2 max-w-xl text-sm font-medium leading-relaxed text-zinc-200 md:text-base">{AI_SEC_HERO.lead}</p>
              <p className="mb-5 max-w-xl text-sm leading-relaxed text-muted">{AI_SEC_HERO.sub}</p>
            ...
            <ServiceProgramHeroVisual
              serviceSlug="technieum-ai-security"
        ...
        <div className="container">
          <div className="container-inner-5xl">
            <h2 className="mb-1.5 text-[0.65rem] font-bold uppercase tracking-[0.16em] text-brand sm:text-xs md:text-[0.8125rem]">
              {AI_SEC_KEY_BENEFITS.title}
            </h2>
            <p className="mb-4 text-sm font-medium text-zinc-500 sm:mb-5">{AI_SEC_KEY_BENEFITS.subtitle}</p>
            <ul className="m-0 grid min-w-0 list-none grid-cols-1 gap-3 p-0 md:grid-cols-2 md:gap-4">
              {AI_SEC_KEY_BENEFITS.items.map((b, index) => {
      ...
      <section className="section-zz-a section-zz-wash-tl bg-bg-inset py-8 md:py-12">
        <div className="container">
          <div className="container-inner-5xl">
            <h2 className="mb-2 text-lg font-semibold text-heading md:text-xl">{AI_SEC_CHALLENGE.title}</h2>
            <p className="mb-4 max-w-3xl text-sm leading-relaxed text-muted">{AI_SEC_CHALLENGE.intro}</p>
            <ul className="m-0 mb-6 max-w-3xl list-none space-y-2.5 p-0">
              {AI_SEC_CHALLENGE.bullets.map((line) => (
            ...
            <h2 className="mb-2 text-lg font-semibold text-heading md:text-xl">{AI_SEC_PARTNER.title}</h2>
            <p className="m-0 max-w-3xl text-sm leading-relaxed text-muted">{AI_SEC_PARTNER.body}</p>
          </div>
        </div>
      </section>
      ...
      <section className="relative overflow-hidden bg-black py-6 md:py-8">
        <div className="container">
          <div className="container-inner-5xl">
            <h2 className="mb-1.5 text-[0.65rem] font-bold uppercase tracking-[0.16em] text-brand sm:text-xs md:text-[0.8125rem]">
              {AI_SEC_SOLUTION_INTRO.title}
            </h2>
            <p className="mb-2 text-sm font-medium text-zinc-500">{AI_SEC_SOLUTION_INTRO.subtitle}</p>
            <p className="mb-4 max-w-3xl text-sm leading-relaxed text-muted sm:mb-5">{AI_SEC_SOLUTION_INTRO.intro}</p>
            <ol className="m-0 grid min-w-0 list-none grid-cols-1 gap-3 p-0 md:grid-cols-2 md:gap-4">
              {AI_SEC_SERVICE_CATALOG.map((s) => {
      ...
      <section className="relative overflow-hidden bg-black py-6 md:py-8">
        <div className="container">
          <div className="container-inner-5xl">
            ...
            <p className="mb-2 text-sm font-medium text-zinc-500">{AI_SEC_EDGE.subtitle}</p>
            <p className="mb-4 max-w-3xl text-sm leading-relaxed text-muted sm:mb-5">{AI_SEC_EDGE.methodologyLead}</p>
            <ul className="m-0 grid min-w-0 list-none grid-cols-1 gap-3 p-0 md:grid-cols-2 md:gap-4">
              {AI_SEC_EDGE.pillars.map((p, index) => {
                const watermark = String(index + 1).padStart(2, '0')
                return (
                  <li
                    key={p.title}
      ...
      <section className="section-zz-b section-zz-wash-br py-8 md:py-12">
        <div className="container">
          <div className="container-inner-5xl">
            ...
            <p className="mb-1.5 text-sm font-medium text-brand">{AI_SEC_INDUSTRY_VERTICALS.subtitle}</p>
            <p className="mb-5 max-w-3xl text-sm leading-relaxed text-muted">{AI_SEC_INDUSTRY_VERTICALS.intro}</p>
            <ul className="m-0 grid list-none gap-4 p-0 sm:grid-cols-2">
              {AI_SEC_INDUSTRY_VERTICALS.verticals.map((v) => (
                <li
                  key={v.title}
                  className="rounded-xl border border-border bg-panel/30 p-5 shadow-[inset_0_1px_0_rgb(255_255_255/0.04)]"
                >
                  <h3 className="mb-2 text-sm font-bold text-heading">{v.title}</h3>
                  <p className="m-0 text-sm leading-relaxed text-muted">{v.body}</p>
                </li>
              ))}
            </ul>
          </div>
      ...
      <ProgramMethodologySection content={AI_SEC_METHODOLOGY} />

      <section className="relative overflow-hidden bg-black py-6 md:py-8">
        <div className="container min-w-0">
          <div className="container-inner-5xl min-w-0">
            <h2 className="mb-1.5 text-[0.65rem] font-bold uppercase tracking-[0.16em] text-brand sm:text-xs md:text-[0.8125rem]">
              {AI_SEC_DELIVERABLE_REPORTS.subtitle}
            </h2>
            <p className="mb-2 text-sm font-medium text-zinc-500">{AI_SEC_DELIVERABLE_REPORTS.title}</p>
            <p className="mb-4 max-w-3xl text-pretty text-sm leading-relaxed text-muted sm:mb-5">
              {AI_SEC_DELIVERABLE_REPORTS.intro}
            </p>
            <ul className="m-0 grid min-w-0 list-none grid-cols-1 gap-3 p-0 md:grid-cols-2 md:gap-4">
              {AI_SEC_DELIVERABLE_REPORTS.pdfCore.map((d, index) => {
                const watermark = String(index + 1).padStart(2, '0')
                return (
                  <li
                    key={d.title}
            ...
            <div className="mt-6 border-t border-white/[0.06] pt-6 md:mt-7 md:pt-7">
              ...
              <ul className="m-0 grid min-w-0 list-none grid-cols-1 gap-3 p-0 md:grid-cols-2 md:gap-4">
                {programPortalLines.map((line, index) => {
                  ...
                  return (
                    <li
                      key={`ai-deliverable-program-${index}`}
                      className="group relative flex min-h-0 min-w-0 overflow-hidden rounded-xl border border-white/[0.07] bg-[#111111] shadow-[0_1px_0_rgb(255_255_255/0.04)_inset] transition-[transform,box-shadow,border-color,background-color] duration-300 hover:-translate-y-0.5 hover:border-white/[0.11] hover:bg-[#131313] hover:shadow-[0_16px_40px_-24px_rgba(0,0,0,0.85)]"
                    >
                      <div
                        className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
      ...
      <section className="section-zz-b section-zz-wash-br py-8 md:py-12">
        <div className="container min-w-0">
          <div className="container-inner-5xl min-w-0 text-left">
            <h2 className="mb-2 text-lg font-semibold text-heading md:text-xl">{AI_SEC_CTA.title}</h2>
            <p className="mb-5 max-w-3xl text-pretty text-sm leading-relaxed text-muted md:mb-6">{AI_SEC_CTA.body}</p>
