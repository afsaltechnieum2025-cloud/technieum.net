...
import { ToipPortalLaunchUI } from '../components/home/ToipPortalLaunchUI'
...
export function ToipProductPage({ doc }: { doc: ProductDocument }) {
  const pitch = PRODUCT_PITCH_PAGES.toip
  const pdf = pdfHref(doc.pdfFile)
  const reduceMotion = useReducedMotion()
  const motion = !reduceMotion
  const chrome = PRODUCT_SCREENSHOT_CHROME.toip
    ...
    <main id="main-content" className="flex flex-col bg-page">
        ...
        <div className="container relative z-10">
          <div className="grid grid-cols-1 gap-x-5 gap-y-6 sm:gap-x-6 md:gap-x-7 lg:grid-cols-2 lg:items-start lg:gap-x-8 lg:gap-y-5 xl:gap-x-10">
            <div className="min-w-0">
              ...
              <p className="mb-2 max-w-4xl text-base font-semibold leading-snug text-heading md:text-lg">{HOME_TOIP.title}</p>
              <p className="mb-6 max-w-3xl text-base leading-snug text-brand md:text-lg lg:mb-8">{HOME_TOIP.subtitle}</p>

              <p className="m-0 max-w-3xl text-sm leading-relaxed text-muted md:text-base">{TOIP_HERO.lead}</p>
            </div>
            <div className="mx-auto w-full max-w-md min-w-0 sm:max-w-lg lg:mx-0 lg:w-full lg:max-w-none">
              ...
              >
                <ToipPortalLaunchUI loop compact />
              </PortalPreviewFrame>
            </div>
      ...
      <section className="section-zz-a section-zz-wash-tl bg-panel/15 pt-14 md:pt-20 pb-8 md:pb-10">
        <div className="container">
          ...
          <p className="mb-8 text-sm leading-relaxed text-muted md:text-base">{TOIP_INTERACTION.intro}</p>
          <ol className="m-0 mb-10 list-decimal space-y-4 pl-5 text-sm leading-relaxed text-muted marker:text-brand md:text-base lg:grid lg:grid-cols-2 lg:gap-x-12 lg:gap-y-5 lg:space-y-0">
            {TOIP_INTERACTION.steps.map((s, i) => (
              <li key={i} className="toip-reveal pl-1" style={revealStyle(i, !motion)}>
                {s}
              </li>
            ))}
          </ol>
          <p className="mb-4 text-xs font-bold uppercase tracking-[0.14em] text-muted">Example queries</p>
          <div className="flex flex-wrap gap-2">
            {TOIP_INTERACTION.exampleQueries.map((q, i) => (
              <span
                key={q}
      ...
      <section className="section-zz-b section-zz-wash-br pt-8 md:pt-10 pb-14 md:pb-20">
        <div className="container">
          ...
          <p className="mb-0 text-sm leading-relaxed text-muted md:text-base">{TOIP_PIPELINE.subtitle}</p>
          <ProductPipelinePhases
            phases={TOIP_PIPELINE.phases}
            ariaLabel="Pipeline from query to final answer"
          />
        </div>
      </section>
      ...
      <section className="section-zz-a section-zz-wash-tl py-14 md:py-20">
        <div className="container">
          ...
          <p className="mb-10 max-w-3xl text-sm leading-relaxed text-muted md:text-base">{TOIP_DATA_MODEL.intro}</p>
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {TOIP_DATA_MODEL.sources.map((s, i) => (
              <article
                key={s.name}
      ...
      <section className="section-zz-b section-zz-wash-tl pt-8 md:pt-10 pb-14 md:pb-20">
        <div className="container">
          <div className="toip-reveal mx-auto max-w-3xl text-center lg:mx-0 lg:max-w-none lg:text-left">
