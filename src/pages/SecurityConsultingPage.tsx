...
export function SecurityConsultingPage() {
  const { visualNodes, visualAriaLabel, pillars, deliverables } = securityConsultingHubConfig
  const programPortalLines = [...deliverables, ...SEC_CONS_DELIVERABLES_PROGRAM_EXTRA]
  ...
  return (
    <main id="main-content" className="flex flex-col bg-page">
      <section className="section-zz-a section-zz-wash-tl relative overflow-hidden bg-bg-inset py-10 md:py-14">
        ...
        <div className="container relative z-10">
          <div className="grid items-center gap-6 lg:grid-cols-2 lg:gap-8">
            <div>
              <p className="text-shimmer-brand mb-2 text-xs font-semibold tracking-wide md:text-sm">{SEC_CONS_HERO.eyebrow}</p>
              <h1 className="heading-scale-hero mb-3 max-w-xl text-pretty">{SEC_CONS_HERO.title}</h1>
              <p className="mb-2 max-w-xl text-sm font-medium leading-relaxed text-zinc-200 md:text-base">{SEC_CONS_HERO.lead}</p>
              <p className="mb-5 max-w-xl text-sm leading-relaxed text-muted">{SEC_CONS_HERO.sub}</p>
        ...
        <div className="container">
          <div className="container-inner-5xl">
            <h2 className="mb-1.5 text-[0.65rem] font-bold uppercase tracking-[0.16em] text-brand sm:text-xs md:text-[0.8125rem]">
              {SEC_CONS_KEY_BENEFITS.title}
            </h2>
            <p className="mb-4 text-sm font-medium text-zinc-500 sm:mb-5">{SEC_CONS_KEY_BENEFITS.subtitle}</p>
            <ul className="m-0 grid min-w-0 list-none grid-cols-1 gap-3 p-0 md:grid-cols-2 md:gap-4">
              {SEC_CONS_KEY_BENEFITS.items.map((b, index) => {
      ...
      <section className="section-zz-a section-zz-wash-tl bg-bg-inset py-8 md:py-12">
        <div className="container">
          <div className="container-inner-5xl">
            <h2 className="mb-2 text-lg font-semibold text-heading md:text-xl">{SEC_CONS_CHALLENGE.title}</h2>
            <p className="mb-4 max-w-3xl text-sm leading-relaxed text-muted">{SEC_CONS_CHALLENGE.intro}</p>
            ...
            <h2 className="mb-2 text-lg font-semibold text-heading md:text-xl">{SEC_CONS_PARTNER.title}</h2>
      ...
      <section className="relative overflow-hidden bg-black py-6 md:py-8">
        <div className="container">
          <div className="container-inner-5xl">
            <h2 className="mb-1.5 text-[0.65rem] font-bold uppercase tracking-[0.16em] text-brand sm:text-xs md:text-[0.8125rem]">
              {SEC_CONS_SOLUTION_INTRO.title}
            </h2>
            <p className="mb-2 text-sm font-medium text-zinc-500">{SEC_CONS_SOLUTION_INTRO.subtitle}</p>
            <p className="mb-4 max-w-3xl text-sm leading-relaxed text-muted sm:mb-5">{SEC_CONS_SOLUTION_INTRO.intro}</p>
      ...
      <section className="relative overflow-hidden bg-black py-6 md:py-8">
        <div className="container">
          <div className="container-inner-5xl">
            ...
            <p className="mb-2 text-sm font-medium text-zinc-500">{SEC_CONS_EDGE.subtitle}</p>
            <p className="mb-4 max-w-3xl text-sm leading-relaxed text-muted sm:mb-5">{SEC_CONS_EDGE.methodologyLead}</p>
            <ul className="m-0 grid min-w-0 list-none grid-cols-1 gap-3 p-0 md:grid-cols-2 md:gap-4">
              {SEC_CONS_EDGE.pillars.map((p, index) => {
                const watermark = String(index + 1).padStart(2, '0')
                return (
                  <li
                    key={p.title}
      ...
      <ProgramMethodologySection content={SEC_CONS_METHODOLOGY} />

      <section className="relative overflow-hidden bg-black py-6 md:py-8">
        <div className="container min-w-0">
          <div className="container-inner-5xl min-w-0">
            <h2 className="mb-1.5 text-[0.65rem] font-bold uppercase tracking-[0.16em] text-brand sm:text-xs md:text-[0.8125rem]">
              {SEC_CONS_DELIVERABLE_REPORTS.subtitle}
            </h2>
            <p className="mb-2 text-sm font-medium text-zinc-500">{SEC_CONS_DELIVERABLE_REPORTS.title}</p>
            <p className="mb-4 max-w-3xl text-pretty text-sm leading-relaxed text-muted sm:mb-5">
              {SEC_CONS_DELIVERABLE_REPORTS.intro}
            </p>
            <ul className="m-0 grid min-w-0 list-none grid-cols-1 gap-3 p-0 md:grid-cols-2 md:gap-4">
              {SEC_CONS_DELIVERABLE_REPORTS.pdfCore.map((d, index) => {
                const watermark = String(index + 1).padStart(2, '0')
                return (
                  <li
                    key={d.title}
            ...
            <div className="mt-6 border-t border-white/[0.06] pt-6 md:mt-7 md:pt-7">
              ...
              <ul className="m-0 grid min-w-0 list-none grid-cols-1 gap-3 p-0 md:grid-cols-2 md:gap-4">
                {programPortalLines.map((line, index) => {
                  ...
                  return (
                    <li
                      key={`sec-cons-deliverable-program-${index}`}
                      className="group relative flex min-h-0 min-w-0 overflow-hidden rounded-xl border border-white/[0.07] bg-[#111111] shadow-[0_1px_0_rgb(255_255_255/0.04)_inset] transition-[transform,box-shadow,border-color,background-color] duration-300 hover:-translate-y-0.5 hover:border-white/[0.11] hover:bg-[#131313] hover:shadow-[0_16px_40px_-24px_rgba(0,0,0,0.85)]"
                    >
                      <div
                        className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
      ...
      <section className="section-zz-b section-zz-wash-br py-8 md:py-12">
        <div className="container min-w-0">
          <div className="container-inner-5xl min-w-0 text-left">
            <h2 className="mb-2 text-lg font-semibold text-heading md:text-xl">{SEC_CONS_CTA.title}</h2>
            <p className="mb-5 max-w-3xl text-pretty text-sm leading-relaxed text-muted md:mb-6">{SEC_CONS_CTA.body}</p>
