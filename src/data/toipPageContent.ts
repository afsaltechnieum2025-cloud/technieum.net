...
export const TOIP_INTERACTION = {
  title: 'How users interact with TOIP',
  intro:
...
export const TOIP_DATA_MODEL = {
  ...
  intro:
    'TOIP does not scrape the internet in real time. It uses a curated, pre-processed intelligence database built once (or periodically) and then queried at runtime. This ensures consistent quality, fast retrieval, and full control, which is critical for professional and compliance-sensitive use.',
  sources: [
    {
...
export const TOIP_RAG = {
  ...
  points: [
    'Embedding model (sentence-transformers): pre-trained all-MiniLM-L6-v2 converts text into 384-dimensional vectors. Used as-is; no fine-tuning in TOIP.',
    ...
    'External LLM: state-of-the-art large language model accessed via API provides reasoning; TOIP augments it with retrieved context so every answer is grounded in the intelligence database.',
  ],
} as const
...
export const TOIP_CLOSING = {
  paragraph:
    'TOIP combines a curated multi-source intelligence database with a flexible external LLM and a retrieval pipeline tuned for offensive security and vulnerability research. The platform delivers detailed, citation-backed, actionable answers that analysts can trust and use immediately.',
} as const
